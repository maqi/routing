...
impl PeerManager {
    ...
    pub fn potential_tunnel_nodes(&self) -> Vec<(XorName, PeerId)> {
        let closest_nodes = self.routing_table
            .other_closest_names(self.our_public_id.name(), self.routing_table.len())
            .unwrap_or_default();
        closest_nodes.iter().filter_map(|name| {
                self.peer_map.get_by_name(name).map_or(None, |peer|
                    if peer.state.can_tunnel_for() {
                        peer.peer_id().map_or(None, |peer_id| Some((**name, *peer_id)))                        
                    } else {
                        None
                    })
            })
